...
export function useTasks(settings, glossary, i18n) {
    ...
    const inferDxfSourceLang = (targetLang) => {
        const value = String(targetLang || '').toLowerCase();
        if (!value || value === 'custom') return 'auto';
        if (value.includes('chinese') || value.includes('中文') || value.includes('zh')) return 'en';
        return 'zh';
    };
    ...
    const buildPayload = () => {
        ...
        if (form.workflow_type === 'markdown_based') {
            ...
            } else if (['dxf', 'dwg'].includes(form.workflow_type)) {
                const filterNonTranslatable = params.filter_non_translatable !== false;
                params.source_lang = inferDxfSourceLang(basePayload.to_lang);
                params.filter_empty = true;
                params.filter_number = filterNonTranslatable;
                params.filter_symbol = filterNonTranslatable;
                params.filter_code = filterNonTranslatable;
                params.filter_target_lang = params.filter_target_lang !== false;
                params.filter_non_source_lang = true;
                params.ai_filter_enable = params.ai_filter_enable === true;
                params.ai_filter_prompt = params.ai_filter_prompt?.trim() || null;
                if (form.workflow_type === 'dwg') {
                    params.oda_path = params.oda_path?.trim() || null;
                    params.oda_timeout = parseInt(params.oda_timeout) || 300;
                    params.dxf_output_version = params.dxf_output_version || 'ACAD2018';
                    params.dwg_output_version = params.dwg_output_version || 'ACAD2007';
                    params.dwg_output_versions = [params.dwg_output_version];
                }
                delete params.filter_non_translatable;
            }
