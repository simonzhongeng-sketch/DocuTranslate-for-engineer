...
export const DEFAULT_DXF_AI_FILTER_PROMPT = 'You are a CAD engineering drawing text filter. For each input segment, decide whether it should be translated. Output exactly KEEP if the segment should be translated, or SKIP if it should not be translated. Do not translate the text. Use a conservative rule: when unsure, output KEEP so useful text is not accidentally skipped. KEEP table headers, column headers, row labels, drawing titles, natural-language notes, labels, descriptions, and other readable content that benefits from translation. KEEP uppercase table headers and acronyms when they act as headers or labels, unless the text is already in the target language. SKIP pure numbers, symbols, drawing numbers, device tags, model numbers, terminal IDs, parameters, units, electrical codes, or text already in the target language. Preserve the one-to-one segment order and return only KEEP or SKIP for each segment.';
...
export function useSettings() {
    ...
    const workflowParams = reactive({
        ...
        dxf: {
            ...
            clean_text: true,
            filter_text: true,
            filter_non_translatable: true,
            filter_empty: true,
            filter_number: true,
            filter_symbol: true,
            filter_code: true,
            filter_target_lang: true,
            filter_non_source_lang: true,
            ai_filter_enable: true,
            ai_filter_prompt: DEFAULT_DXF_AI_FILTER_PROMPT
        },
        dwg: {
            insert_mode: 'replace',
            separator: '\\n',
            ...
            dwg_output_version: 'ACAD2007',
            dwg_output_versions: ['ACAD2007']
        }
    });
    ...
    const loadWorkflowParams = () => {
        ['txt', 'xlsx', 'docx', 'srt', 'epub', 'html', 'ass', 'pptx', 'dxf', 'dwg'].forEach(t => {
        ...
        workflowParams.dxf.clean_text = storage.getBool('translator_dxf_clean_text', true);
        workflowParams.dxf.filter_text = storage.getBool('translator_dxf_filter_text', true);
        workflowParams.dxf.source_lang = 'auto';
        workflowParams.dxf.filter_non_translatable = storage.getBool('translator_dxf_filter_non_translatable', true);
        workflowParams.dxf.filter_empty = true;
        workflowParams.dxf.filter_number = true;
        workflowParams.dxf.filter_symbol = true;
        workflowParams.dxf.filter_code = true;
        workflowParams.dxf.filter_target_lang = storage.getBool('translator_dxf_filter_target_lang', true);
        workflowParams.dxf.filter_non_source_lang = true;
        workflowParams.dxf.ai_filter_enable = storage.getBool('translator_dxf_ai_filter_enable', true);
        workflowParams.dxf.ai_filter_prompt = storage.get('translator_dxf_ai_filter_prompt', DEFAULT_DXF_AI_FILTER_PROMPT) || DEFAULT_DXF_AI_FILTER_PROMPT;
        workflowParams.dwg.clean_text = storage.getBool('translator_dwg_clean_text', true);
        workflowParams.dwg.filter_text = storage.getBool('translator_dwg_filter_text', true);
        workflowParams.dwg.source_lang = 'auto';
        workflowParams.dwg.filter_non_translatable = storage.getBool('translator_dwg_filter_non_translatable', true);
        workflowParams.dwg.filter_empty = true;
        workflowParams.dwg.filter_number = true;
        workflowParams.dwg.filter_symbol = true;
        workflowParams.dwg.filter_code = true;
        workflowParams.dwg.filter_target_lang = storage.getBool('translator_dwg_filter_target_lang', true);
        workflowParams.dwg.filter_non_source_lang = true;
        workflowParams.dwg.ai_filter_enable = storage.getBool('translator_dwg_ai_filter_enable', true);
        workflowParams.dwg.ai_filter_prompt = storage.get('translator_dwg_ai_filter_prompt', DEFAULT_DXF_AI_FILTER_PROMPT) || DEFAULT_DXF_AI_FILTER_PROMPT;
        workflowParams.dwg.oda_path = storage.get('translator_dwg_oda_path', '');
        workflowParams.dwg.oda_timeout = storage.getNum('translator_dwg_oda_timeout', 300);
        workflowParams.dwg.dxf_output_version = storage.get('translator_dwg_dxf_output_version', 'ACAD2018');
        workflowParams.dwg.dwg_output_version = storage.get('translator_dwg_dwg_output_version', 'ACAD2007');
        workflowParams.dwg.dwg_output_versions = [workflowParams.dwg.dwg_output_version || 'ACAD2007'];
    };
    ...
    // ===== Default Workflow =====
    const DEFAULT_EXTENSIONS = ['pdf','png','jpg','jpeg','gif','bmp','webp','txt','md','docx','doc','xlsx','csv','xls','epub','pptx','ppt','dxf','dwg','srt','ass','json','html','htm'];
