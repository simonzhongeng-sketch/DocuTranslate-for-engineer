...
onMounted(async () => {
    ...
    try {
        ...
        version.value = backendMeta.version;
    ...
    if (!localStorage.getItem('translator_dwg_oda_path') && backendMeta?.oda_file_converter?.path) {
        workflowParams.dwg.oda_path = backendMeta.oda_file_converter.path;
    }
    ...
    if (window.location.pathname.includes('/admin')) {
        document.title = "DocuTranslate for engineer - Admin Panel";
        try {
