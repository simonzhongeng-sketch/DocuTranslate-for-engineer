...
from docutranslate.workflow.oda_converter import ODA_DOWNLOAD_URL, OdaFileConverter
...
app = FastAPI(
    ...
    redoc_url=None,
    lifespan=lifespan,
    title="DocuTranslate for engineer API",
    description=f"""
DocuTranslate for engineer 后端服务 API，提供文档翻译、工程图纸翻译、状态查询、结果下载等功能。
...
- **工作流选择**: `payload.workflow_type` 决定任务类型（如 `markdown_based`, `txt`, `json`, `xlsx`, `docx`, `srt`, `epub`, `html`, `ass`, `pptx`, `dxf`, `auto`）。
...
async def service_download_file(
        ...
        file_type: FileType = FastApiPath(
            ...
            examples=["html", "json", "csv", "docx", "srt", "epub", "ass", "pptx", "dxf"],
        ),
):
...
async def service_get_app_version():
    oda_path = None
    try:
        oda_path = str(OdaFileConverter.resolve_executable())
    except Exception:
        oda_path = None
...
async def service_flat_translate(
        ...
        workflow_type: str = Form("auto",
                                  description="工作流类型: auto, markdown_based, txt, json, xlsx, docx, srt, epub, html, ass, pptx, dxf, dwg"),
        ...
        mineru_deploy_server_url: Optional[str] = Form("", description="[MinerU Local] Server URL"),
        source_lang: str = Form("auto", description="[DXF] Source language hint"),
        clean_text: bool = Form(True, description="[DXF] Clean text before translation"),
        filter_text: bool = Form(True, description="[DXF] Enable text filtering"),
        filter_empty: bool = Form(True, description="[DXF] Filter empty text"),
        filter_number: bool = Form(True, description="[DXF] Filter numbers and parameters"),
        filter_symbol: bool = Form(True, description="[DXF] Filter symbol-only text"),
        filter_code: bool = Form(True, description="[DXF] Filter engineering tags and codes"),
        filter_target_lang: bool = Form(True, description="[DXF] Filter text already in target language"),
        filter_non_source_lang: bool = Form(True, description="[DXF] Filter text outside source language"),
        ai_filter_enable: bool = Form(True, description="[DXF] Enable AI text filtering"),
        ai_filter_prompt: Optional[str] = Form("", description="[DXF] Custom AI text filtering prompt"),
        oda_path: Optional[str] = Form("", description="[DWG] Path to ODAFileConverter.exe"),
        oda_timeout: int = Form(300, description="[DWG] ODA conversion timeout in seconds"),
        dwg_output_version: str = Form("ACAD2007", description="[DWG] Output DWG version"),
        dwg_output_versions: Optional[List[str]] = Form(None, description="[DWG] Output DWG versions"),
        dxf_output_version: str = Form("ACAD2018", description="[DWG] Intermediate DXF version"),
    ...
    payload_dict = {
        ...
        "filter_target_lang": filter_target_lang,
        "filter_non_source_lang": filter_non_source_lang,
        "ai_filter_enable": ai_filter_enable,
        "ai_filter_prompt": ai_filter_prompt,
        "oda_path": oda_path,
        "oda_timeout": oda_timeout,
        "dwg_output_version": dwg_output_version,
        "dwg_output_versions": dwg_output_versions,
        "dxf_output_version": dxf_output_version,
...
def run_app(host=None, port: int | None = None, enable_CORS=False,
    ...
    try:
        ...
        print("正在启动 DocuTranslate for engineer WebUI 版本号：v1.0.0")
