...
from docutranslate.workflow.dwg_workflow import DwgWorkflow, DwgWorkflowConfig
...
class TranslationService:
    ...
    async def start_translation(
        ...
        if payload.workflow_type == "auto":
            ...
            universal_fields = [
                ...
                "source_lang", "clean_text", "filter_text", "filter_empty", "filter_number",
                "filter_symbol", "filter_code", "filter_target_lang", "filter_non_source_lang",
                "ai_filter_enable", "ai_filter_prompt", "min_layout_scale",
                "oda_path", "oda_timeout", "dwg_output_version", "dwg_output_versions", "dxf_output_version"
            ]
    ...
    def _create_workflow(
        ...
        if isinstance(payload, MarkdownWorkflowParams):
        ...
        elif isinstance(payload, DwgWorkflowParams):
            task_logger.info("Creating DwgWorkflow config.")
            translator_args = payload.model_dump(
                include={
                    "skip_translate",
                    "base_url",
            ...
            translator_args["glossary_generate_enable"] = payload.glossary_generate_enable
            translator_args["glossary_agent_config"] = build_glossary_agent_config()
            translator_config = DxfTranslatorConfig(**translator_args)
            translator_config.progress_tracker = progress_tracker

            versions = tuple(payload.dwg_output_versions or [payload.dwg_output_version or "ACAD2007"])
            workflow_config = DwgWorkflowConfig(
                translator_config=translator_config,
                oda_path=payload.oda_path or None,
                oda_timeout=payload.oda_timeout,
                dwg_output_versions=versions,
                dxf_output_version=payload.dxf_output_version,
                logger=task_logger,
                progress_tracker=progress_tracker,
            )
            return DwgWorkflow(config=workflow_config)

        elif isinstance(payload, DxfWorkflowParams):
            task_logger.info("Creating DxfWorkflow config.")
            translator_args = payload.model_dump(
                include={
                    "skip_translate",
                    "base_url",
            ...
            workflow_config = DxfWorkflowConfig(
                translator_config=translator_config,
                logger=task_logger,
                progress_tracker=progress_tracker,
            )
            return DxfWorkflow(config=workflow_config)
    ...
    def _build_export_map(
        ...
        if isinstance(workflow, DxfExportable):
            export_map["dxf"] = (
                workflow.export_to_dxf,
                f"{filename_stem}_translated.dxf",
                False,
            )
        if isinstance(workflow, DwgWorkflow):
            for version in workflow.config.dwg_output_versions:
                version_key = version.lower()
                export_map[f"dwg_{version_key}"] = (
                    lambda version=version: workflow.export_to_dwg(version),
                    f"{filename_stem}_translated_{version}.dwg",
                    False,
                )
